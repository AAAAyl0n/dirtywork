...
export default function ArxivSwitch() {
  const router = useRouter()
  const pathname = usePathname()
  const [isChecked, setIsChecked] = useState(pathname === '/blog/key-person')

  useEffect(() => {
    const isKeyPersonPath = pathname === '/blog/key-person'
    setIsChecked(isKeyPersonPath)

    if (!isKeyPersonPath) {
      router.prefetch('/blog/key-person')
    } else {
  ...
  const handleCheckedChange = useCallback(() => {
    ...
    setTimeout(() => {
      const newPath = pathname === '/blog/key-person' ? '/blog' : '/blog/key-person'
      router.push(newPath)
    }, 300)
  }, [pathname, router])
  ...
  return (
    <div className="flex items-center gap-x-2">
      ...
      <label
        ...
        htmlFor="key-person-switch"
      >
        {isChecked ? 'Key Person' : 'Articles'}
      </label>
    </div>
  )
}
